...
function purge_old_wfruns () {
  ...
  SECONDS=0
  local LOGTOPIC=' purge-old-workflow-runs:'
  ...
  [ -n "$OLD" ] || return 4$(
    echo E:$LOGTOPIC 'Missing option: older_than' >&2)
  [ -z "${OLD//[A-Za-z0-9:. -]/}" ] || return 4$(
    echo E:$LOGTOPIC 'Option "older_than" contains unsupported characters.' \
    'Acceptable characters are letters (A-Z, a-z), digits (0-9),' \
    'colon (:), full stop (.), space and minus (-).' >&2)
  OLD="$(date +'%FT%TZ' --utc --date="$OLD")"
  [ -n "$OLD" ] || return 4$(echo E:$LOGTOPIC >&2 \
    'Unsupported date/time format in option "older_than".')
  echo D:$LOGTOPIC "Effective older_than date: $OLD"

  E+=" Config error: Option"
  local OVERALL_TIMEOUT="${CFG[overall_timeout_seconds]}"
  value_mustbe_simple_integer "E:$LOGTOPIC overall_timeout_seconds" \
    "$OVERALL_TIMEOUT" ge:1 || return $?

  value_mustbe_simple_integer "E:$LOGTOPIC api_cooldown_seconds" \
    "${CFG[api_cooldown_seconds]}" ge:1 || return $?
  value_mustbe_simple_integer "E:$LOGTOPIC max_failed" \
    "${CFG[max_failed]}" ge:0 || return $?
  value_mustbe_simple_integer "E:$LOGTOPIC max_purged" \
  ...
  value_mustbe_simple_integer "E:$LOGTOPIC api_pagesize" \
    "$RUNS_PER_PAGE" ge:1 || return $?

  local CONCLU="${CFG[conclusions]}"
  [ -n "$CONCLU" ] || return 4$(echo "E:$LOGTOPIC conclusions is empty!" >&2)
  [ "${CONCLU// /}" != '*' ] || return 4$(
    echo "E:$LOGTOPIC conclusions must be explicit!" >&2)
  CONCLU=" ${CONCLU} "
  ...
  if [ "$N_TOTAL_RUNS" == 0 ]; then
    VAL="D:$LOGTOPIC No too-old runs were found."
    echo $'\n'"$VAL"$'\n' >>"${GITHUB_STEP_SUMMARY:-/dev/null}"
    echo D: "$VAL"
    return 0
  fi
  [ "${N_TOTAL_RUNS:-0}" -ge 1 ] || return 4$(
    echo E:$LOGTOPIC 'Failed to count results.' >&2)
  ...
  VAL="D:$LOGTOPIC Done. $N_TOTAL_RUNS total, $N_RUNS_PURGED purged, $(
...
function purge_old_wfruns__api_cooldown () {
  [ "$SECONDS" -le "$OVERALL_TIMEOUT" ] || return 4$(echo E:$LOGTOPIC >&2 \
    echo E:$LOGTOPIC 'Abort: Exceeded overall_timeout_seconds.')
  sleep "${CFG[api_cooldown_seconds]}s" || return 4$(echo E:$LOGTOPIC >&2 \
...
function purge_old_wfruns__process_tsv_runs_list () {
  ...
  while IFS= read -r BUF; do
    [ "$N_RUNS_FAILED" -lt "${CFG[max_failed]}" ] || return 4$(
      echo E:$LOGTOPIC "Abort: We have encountered $N_RUNS_FAILED" \
        "≥ max_failed = ${CFG[max_failed]} failures." >&2)
    [ "$N_RUNS_PURGED" -lt "${CFG[max_purged]}" ] || return 4$(
      echo E:$LOGTOPIC "Abort: We have purged $N_RUNS_PURGED" \
    ...
    [ -z "$BUF" ] || return $?$(
      echo E:$LOGTOPIC "Unexpected left-over TSV column data: $BUF" >&2)
    VAL=
    local -p | grep ROW
    ...
    BUF="${BUF//$'\r'/}"
    if [ "$RV" != 0 ]; then
      echo W:$LOGTOPIC "Failed to delete $KEY (rv=$RV)" >&2
      echo W:$LOGTOPIC "API response as base64: $(
    ...
    if [ "${HEAD:0:11}" == 'HTTP/2 404 ' ]; then
      echo D:$LOGTOPIC "Missing in action: $KEY"
      (( N_RUNS_PURGED += 1 ))
      continue
    fi
    if [ -z "$BODY" -a "${HEAD:0:11}" == 'HTTP/2 204 ' ]; then
      echo D:$LOGTOPIC "Deleted: $KEY"
      (( N_RUNS_PURGED += 1 ))
      continue
    fi
    ...
    case "$BODY" in
      '{'*'"message":'*'}'* )
        echo E:$LOGTOPIC 'Unexpected API reply:' \
          "[${HEAD%%$'\n'*}] ${BODY//$'\n'/ }" >&2
        return 4;;
    esac

    echo E:$LOGTOPIC "Unexpected API reply" >&2
    echo W:$LOGTOPIC "API response headers (${#HEAD} bytes) as base64: $(
      base64 --wrap=9009009009 <<<"$HEAD")" >&2
    echo W:$LOGTOPIC "API response body (${#BODY} bytes) as base64: $(
...
function purge_old_wfruns__decide_logtopic () {
  local WF="$GITHUB_WORKFLOW"
  WF="${WF,,}"
  WF="${WF//[_.-]/}"
  # echo D: $FUNCNAME: "WF='$WF'"
  case "$WF" in
    */clean*oldworkflow* | \
    */pruneoldworkflow* | \
    */purgeoldworkflow* | \
    */wipeoldworkflow* | \
    . )
      # Workflows with names like these probably won't do much else,
      # so there's no need to include a (potentially confusing because
      # different) topic in log messages.
      LOGTOPIC=;;
  esac
}
