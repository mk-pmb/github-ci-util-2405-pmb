...
function chdir_relative_and_source () {
  ...
  case "$SELFFILE" in
    proj://* ) SELFFILE="$CI_PROJECT_DIR${SELFFILE#*/}";;
    git-repo://* ) SELFFILE="$(git rev-parse --show-toplevel)${SELFFILE#*/}";;
    ghciu://* ) SELFFILE="$GHCIU_DIR${SELFFILE#*/}";;
  esac
  local VAL="$(readlink -m -- "$SELFFILE")"
  [ -f "$VAL" ] || return 4$(echo E: >&2 "$INVOKED_AS:" \
    "Failed to resolve absolute path to self! Original: $SELFFILE")
  SELFFILE="$VAL"
